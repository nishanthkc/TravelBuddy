...
# SECOND LOGIC - 2
def find_closest_pair(num1, num2, user_destination):
    df = pd.read_csv("askme/static/clean_airport_optimised.csv")
    # get detination IATA CODE
    a = list(df['City'])
    destiny = ""
    to_destination = user_destination.lower()
    if to_destination.lower() in a:
        destiny = df.iloc[a.index(to_destination)]['IATA Code']
    else:
        destiny = user_destination

    # get User's IATA CODE
    list1 = list(df['Latitude Decimal Degrees'])
    list2 = list(df['Longitude Decimal Degrees'])
    ...
    return (df.iloc[list1.index(closest_pair[0])]['IATA Code'], destiny)

def generate_urls(from_iata_code, to_iata_code):
...
#     hotel = "https://www.expedia.co.in/Hotel-Search?destination={}&selected=&d1={}&startDate={}&d2={}&endDate={}&adults=2".format(str(to_destination), today_date, today_date, tomorrow_date, tomorrow_date)
    ...
    flight = "https://www.expedia.co.in/Flights-Search?flight-type=on&mode=search&trip=oneway&leg1=from:{},to:{},departure:{}TANYT&options=cabinclass:economy&fromDate={}&d1={}&passengers=adults:1,infantinlap:N".format(from_iata_code, to_iata_code, departure, departure, tomorrow_date)
    return (flight)
